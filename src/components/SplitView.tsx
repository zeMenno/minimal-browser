    ...
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-[#566174]">
        ...
        <div className="text-sm">
          Press <kbd className="rounded bg-[#1d2430] px-1.5 py-0.5 text-xs">Ctrl+T</kbd> for a new
          tab or <kbd className="rounded bg-[#1d2430] px-1.5 py-0.5 text-xs">Ctrl+K</kbd> for the
          command palette
        </div>
      </div>
    );
        ...
        <div key={child.id} className="contents">
          {i > 0 && (
            <div
              ...
              className={`shrink-0 bg-[#161b24] transition-colors hover:bg-[#7aa2f7] ${
...
function Pane({ node }: { node: LeafNode }) {
  ...
  // Report this pane's rectangle to the main process so it can position the
  // native WebContentsView over it.
  useEffect(() => {
  ...
  return (
    <div
      className={`relative m-[2px] flex min-h-0 min-w-0 flex-1 overflow-hidden rounded border ${
        isActive ? "border-[#3b4d6b]" : "border-[#1c232e]"
      }`}
    ...
    >
      ...
      <div
        ref={contentRef}
        className="flex h-full w-full flex-col items-center justify-center gap-2 bg-[#10151d]"
      >
        {tab.favicon ? (
          <img src={tab.favicon} alt="" className="h-6 w-6 opacity-60" />
        ) : (
          <div className="h-6 w-6 rounded bg-[#1d2430]" />
        )}
  ...
  return (
    <div
      ...
      className={`absolute rounded border-2 border-dashed transition-colors ${className} ${
        isOver ? "border-[#7aa2f7] bg-[#7aa2f7]/15" : "border-transparent"
      }`}
    />
  );
}
